...
| ADR-0013 | [Authorization Model — Three-Layer RBAC + Permissions](0013-authorization.md) | Accepted | Three-layer: `JwtGuard` (identity) → `PermissionsGuard` (roles + permissions from JWT) → Domain policies (resource ownership). Permissions embedded in JWT payload for O(1) checks. | `docs/standards/security.md` | Auth, Tag, Quiz, Category, Social, Discussion, Tournament | `docs/architecture/authorization-flow.md` |
...
| ADR-0017 | [Counter Reconciliation — Mutate-In-Transaction or Full-Recompute](0017-counter-reconciliation.md) | Accepted | Every denormalized counter is either mutated inside the source transaction (Strategy A) or never written and always recomputed from source by a service method (Strategy B). All counters ship a daily reconciliation sweep and a production-guarded backfill. | `docs/standards/database.md` | Quiz, Review, Discussion, Tournament, Ranking, Bookmark | `docs/architecture/persistence-flow.md` |
