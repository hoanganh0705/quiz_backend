...
| ADR-0018 | [Ports and Adapters — Hexagonal Architecture](0018-ports-and-adapters.md) | Accepted | All cross-cutting and external dependencies are hidden behind TypeScript port interfaces (`STORAGE_PORT`, `CACHE_PROVIDER`, …) and bound to concrete adapters by NestJS DI. Domain layer can only see the port contracts, not the implementation. | `docs/standards/architecture.md` | All modules | `docs/architecture/overview.md` |
| ADR-0019 | [Transactional Outbox for Cross-Domain Events](0019-transactional-outbox.md) | Accepted | Domain mutation and outbox event commit in the same Postgres transaction. `LISTEN/NOTIFY` for sub-second dispatch; 30-second fallback poll for resilience. At-least-once delivery; consumers MUST be idempotent. | `docs/standards/architecture.md` | Auth, Quiz, Attempt, Notification | `docs/architecture/event-flow.md` |
| ADR-0020 | [Optimistic Locking on Quiz Instances](0020-optimistic-locking.md) | Accepted | Every `quiz_instances` row carries a `version` int. Updates are conditional on the version the writer observed; the helper in `quiz-instance.repository.ts#updateWithVersion` surfaces `InstanceOptimisticLockError` on conflict. | `docs/standards/database.md` | Instance, Attempt | `docs/architecture/persistence-flow.md` |
| ADR-0021 | [UUIDv7 for All Primary Keys](0021-uuidv7-primary-keys.md) | Accepted | All new tables use UUIDv7 primary keys. The time-ordered bits make `created_at`-range queries use the primary-key index. Predicate and tooling notes; a follow-up backfill is planned for legacy tables. | `docs/standards/architecture.md` | All modules | `docs/architecture/persistence-flow.md` |
| ADR-0022 | [Stampede Protection in the Cache Layer](0022-stampede-protection.md) | Accepted | Read-through cache for hot keys uses a Redis-backed lock (`SET NX PX`) to coalesce concurrent misses. First miss fetches; subsequent misses poll. Lock TTL bounds stuck owners; 500 ms retry budget then degrades to direct fetch. | `docs/standards/architecture.md` | Quiz, User, Ranking | `docs/architecture/request-flow.md` |
| ADR-0023 | [Fail-Open Circuit Breaker for Redis](0023-redis-circuit-breaker.md) | Accepted | Every business-critical Redis call is wrapped in a three-state circuit breaker (CLOSED/OPEN/HALF-OPEN). When OPEN, calls short-circuit to a documented fallback. State transitions emit a metric so the on-call can alert. | `docs/standards/architecture.md` | All modules | `docs/architecture/overview.md` |
