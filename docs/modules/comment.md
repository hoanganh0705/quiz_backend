...
Owns the **comment section attached to each quiz**: comments anchored to a quiz, replies (two-level hierarchy only), votes, content reporting, and moderator hide/restore. The comment section is **not** a standalone comment platform.
...
There is no `CommentThread`, no `CommentSubscription`, no `SavedComment`, no `TrendingComment`, no `SolvedComment` anymore.
...
- **Moderation**: only `COMMENT_MODERATE` holders may hide / restore comments and review reports. Authorization is enforced by `CommentAuthorizationPolicy` inside the domain service.
...
| Hide / restore a comment, list reports, review reports | `COMMENT_MODERATE` (Admin, Moderator) |
...
The dependency graph is one-way: only `Notification` depends on `Comment` (for the bus symbol and event types). `Comment` depends on `Quiz` and `User` via their existence ports.
...
The full route map is documented in [`docs/standards/api.md`](../standards/api.md). Summary:
...
`CommentCounterReconcilerService` runs daily at 03:30 to recompute `comments.replies_count` from the underlying rows. Updates are idempotent (`IS DISTINCT FROM`), so a re-run is safe. The reconciler is the only scheduled job in the module.
