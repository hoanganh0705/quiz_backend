...
- `src/common/errors/problem-code-mapping.spec.ts` extended with 7 new resolution assertions.
- `src/modules/instance/domain/errors/instance-domain.errors.ts` migrated. `InstanceDomainError` becomes an abstract namespace marker (no `code`). All 7 concrete classes preserve their existing default messages that reference the module's constants (`instance.constants.ts`). Audit: `grep -rn 'new InstanceDomainError' src/` returns no matches.
- `src/modules/instance/domain/errors/instance-domain.errors.spec.ts` (new): 47 mapping-completeness tests iterating a 7-row table + 5 aggregate invariants. The total-count guard (`total === 7`) defends against accidental additions/removals.
- `src/modules/instance/transport/filters/instance-domain-exception.filter.ts` (deleted): the HTTP per-module `@Catch(InstanceDomainError)` filter is gone.
- `src/modules/instance/transport/filters/ws-exception.filter.ts` (KEPT): the WS gateway filter is independent and only handles `UnauthorizedException` and a catch-all (not domain errors). It is correctly retained because the gateway operates on a different transport context (`host.switchToWs()`).
- `src/modules/instance/dto/response/instance-domain-error.dto.ts` (deleted): the per-module `*DomainErrorDto` Swagger DTO is gone. **This is the fourth (and final) `*DomainErrorDto` deletion in Phase 2** — completing §8.3's "No *DomainErrorDto files remain in src/modules/*/dto/" criterion (verified by `glob src/modules/*/dto/response/*domain-error*.dto.ts` returning 0 files).
- `src/modules/instance/dto/response/index.ts` (updated): removed the `export * from './instance-domain-error.dto'` re-export.
- `src/modules/instance/instance.module.ts`: removed the HTTP filter provider (`InstanceDomainExceptionFilter`) and its import. `WsExceptionFilter` is retained with an updated comment explaining why it stays.
- `src/modules/instance/transport/controller/instance.controller.ts`: removed `UseFilters` import, `InstanceDomainErrorDto` import, `InstanceDomainExceptionFilter` import, `getSchemaPath` import, `ApiExtraModels` import, `@UseFilters(InstanceDomainExceptionFilter)` decoration, `@ApiExtraModels(ProblemDetailDto, InstanceDomainErrorDto)` decoration. The 3 helper decorators (`instanceNotFoundResponse`, `instanceForbiddenResponse`, `instanceBadRequestResponseDual`) are simplified to emit `ProblemDetailDto` directly with `ErrorResponseExamples` wired in. The `instanceBadRequestResponseDual` + `instanceBadRequestResponseValidation` pair was **collapsed** into a single `instanceBadRequestResponse` helper — every 400 response is now uniform RFC 7807, so the dual-schema `oneOf` was unnecessary. The `oneOf` schema references (line 102-104 of the original) are removed. The `instanceUnauthorizedResponse` helper was unchanged. Comments referring to `InstanceDomainErrorDto` are updated to `ProblemDetail`. Comments referring to `InstanceDomainExceptionFilter` are updated to `GlobalExceptionFilter`.
- `test/rfc7807.e2e-spec.ts` extended with 7 instance-coverage tests covering the full 4-status-code matrix.

Wire-shape changes (Phase 2 breaking):
1. **Envelope change.** Instance error responses now follow the canonical RFC 7807 shape. The prior `{ statusCode, message, error }` envelope is fully gone.
2. **`InstanceNotFoundError.detail`** improvement: prior filter hardcoded `'Resource not found'`; now preserves thrown message (default: `'Quiz instance not found'`).
3. **`InstanceNotHostError.detail`** improvement: prior filter hardcoded `'You do not have permission to perform this action'`; now preserves thrown message (default: `'Only the host can perform this action'`).
4. **`InstanceNotOpenError.detail`** improvement: prior filter hardcoded `'Invalid request data'`; now preserves thrown message (default: `'Instance is not open for joining'`).
5. **`InstanceFullError.detail`** improvement: prior filter hardcoded `'Invalid request data'`; now preserves thrown message (default: `'Instance is full'`).
6. **`InstanceAlreadyStartedError.detail`** improvement: prior filter hardcoded `'Invalid request data'`; now preserves thrown message (default: `'Instance has already started'`).
7. **`InstanceAlreadyClosedError.detail`** improvement: prior filter hardcoded `'Invalid request data'`; now preserves thrown message (default: `'Instance is already closed'`).
8. **`PlayerAlreadyJoinedError.detail`** improvement: prior filter hardcoded `'Resource already exists'`; now preserves thrown message (default: `'You have already joined this instance'`).

Decisions:
1. **`InstanceDomainError` stays abstract (no `code`, no mapping entry).** Same pattern as the prior 10 modules.
2. **HTTP filter deleted; WS filter kept.** The two filters are independent: HTTP `InstanceDomainExceptionFilter` operated on `host.switchToHttp()` and was controller-scoped via `@UseFilters`. WS `WsExceptionFilter` operates on `host.switchToWs()` and is gateway-scoped via `@UseFilters` on `InstanceGateway`. Deleting the HTTP filter is safe because the global filter handles HTTP `InstanceDomainError`. The WS filter is retained because (a) it operates on a different context, (b) it handles only auth/generic errors (no domain errors), and (c) Phase 2's scope is HTTP error responses only — the WS gateway's error protocol is out of scope.
3. **`InstanceDomainErrorDto` Swagger DTO deletion.** Fourth (and final) module with such a DTO. **Completes §8.3's "No *DomainErrorDto files remain" criterion** for the entire Phase-2 conversion. The 5 helper decorators in `InstanceController` are simplified. Two helpers (`instanceBadRequestResponseDual`, `instanceBadRequestResponseValidation`) are collapsed into one (`instanceBadRequestResponse`).
4. **`oneOf` schema simplification.** The `schema.oneOf([ProblemDetailDto, InstanceDomainErrorDto])` reference (line 102-104 of the original) is removed. After Phase 2 every error response is uniform RFC 7807, so the dual-schema discriminator was unnecessary. This is the same `oneOf` simplification as tournament, but applied to a `BadRequest` rather than a `NotFound` / `Conflict`.
5. **`PlayerAlreadyJoinedError` is defined but unused.** Audit: `grep -rn 'new PlayerAlreadyJoinedError' src/` returns 0 hits in service. The class is kept (with `code: 'PLAYER_ALREADY_JOINED'` and a mapping entry) as documentation / forward-compatibility. If a future call site throws it, the global filter will resolve the code correctly.
6. **Module-level filter provider removal.** Same as tournament, review, bookmark — filter was registered as a module provider. `instance.module.ts` providers list is shortened; HTTP filter import is removed. `WsExceptionFilter` stays in providers (the gateway needs it).
7. **Logging parity.** Prior filter had no logging (like review, bookmark). The global filter logs `event: 'http_client_error'` (warn) for 4xx and `event: 'http_server_error'` (error) for 5xx. Same severity buckets.
8. **`LEGACY_COMPAT` shim still deferred.** Same rationale as rev4.0 / rev4.1 / rev4.2 / rev4.3 / rev4.4.

Verification: `tsc --noEmit` clean. ESLint clean on every touched file (3 prettier auto-fixes applied). Instance + common-errors unit tests pass (120 tests; +7 mapping + +47 instance spec). rfc7807 e2e suite passes (92 tests; +7 new instance cases). The 8 pre-existing tournament app-spec failures remain unchanged from `main`. `glob src/modules/*/dto/response/*domain-error*.dto.ts` returns 0 files (completing §8.3's criterion).

**v4.4 (2026-07-11).** Phase 2 partial — **bookmark** shipped end-to-end. Fifth Phase-2 module migrated; 7 concrete exceptions → 4 status codes (400/403/404/409). Third module (after tournament, review) with an existing `*DomainErrorDto` Swagger DTO to delete. Concrete deliverables:
- `src/common/errors/problem-code-mapping.ts` extended with 7 BOOKMARK_*/COLLECTION_* entries: 3 × 404 (`BOOKMARK_NOT_FOUND`, `COLLECTION_NOT_FOUND`, `BOOKMARK_COLLECTION_NOT_FOUND`), 1 × 403 (`COLLECTION_FORBIDDEN`), 2 × 409 (`BOOKMARK_CONFLICT`, `COLLECTION_CONFLICT`), 1 × 400 (`BOOKMARK_VALIDATION`).
- `src/common/errors/problem-code-mapping.spec.ts` extended with 7 new resolution assertions.
- `src/modules/bookmark/domain/errors/bookmark-domain.errors.ts` migrated. `BookmarkDomainError` becomes an abstract namespace marker (no `code`). All 7 concrete classes preserve their existing default messages that reference the module's constants (`bookmark.constants.ts`). Audit: `grep -rn 'new BookmarkDomainError' src/` returns no matches.
- `src/modules/bookmark/domain/errors/bookmark-domain.errors.spec.ts` (new): 47 mapping-completeness tests iterating a 7-row table + 5 aggregate invariants. The total-count guard (`total === 7`) defends against accidental additions/removals.
- `src/modules/bookmark/transport/filters/bookmark-domain-exception.filter.ts` (deleted): the per-module `@Catch(BookmarkDomainError)` filter is gone.
- `src/modules/bookmark/dto/response/bookmark-response-docs.dto.ts` (deleted): the per-module `*DomainErrorDto` Swagger DTO is gone. The file's docstring mentioned previously-deleted `Wrapped*Dto` classes but the file in fact contained only `BookmarkDomainErrorDto`.
- `src/modules/bookmark/dto/response/index.ts` (updated): removed the `export * from './bookmark-response-docs.dto'` re-export.
- `src/modules/bookmark/bookmark.module.ts`: removed the filter provider (`BookmarkDomainExceptionFilter`) and its import. Same pattern as tournament, review — filter was module-level.
- `src/modules/bookmark/transport/controller/bookmark.controller.ts`: removed `UseFilters` import, `BookmarkDomainErrorDto` import, `BookmarkDomainExceptionFilter` import, `@UseFilters(BookmarkDomainExceptionFilter)` decoration. The 3 helper decorators (`bookmarkForbiddenResponse`, `bookmarkNotFoundResponse`, `bookmarkConflictResponse`) are simplified to emit `ProblemDetailDto` directly with `ErrorResponseExamples` wired in. The controller has heavy use of these helpers (15+ call sites).
- `test/rfc7807.e2e-spec.ts` extended with 7 bookmark-coverage tests covering the full 4-status-code matrix.

Wire-shape changes (Phase 2 breaking):
1. **Envelope change.** Bookmark error responses now follow the canonical RFC 7807 shape. The prior `{ statusCode, message, error }` envelope is fully gone.
2. **`BookmarkNotFoundError.detail` + `CollectionNotFoundError.detail`** improvement: both were hardcoded `'Resource not found'`; both now preserve thrown messages. The `CollectionNotFoundError` call site at `bookmark-command.service.ts:161` passes `'Quiz not found'` — now surfaces verbatim.
3. **`BookmarkCollectionNotFoundError.detail`** improvement: prior filter hardcoded `'Bookmark collection analytics not found'`, even for distinct throw messages. The call site at `bookmark-command.service.ts:226` passes `'Collection was deleted while processing this request. Please retry.'` — now surfaces verbatim.
4. **`CollectionForbiddenError.detail`** improvement: was hardcoded `'You do not have permission to perform this action'`; now preserves thrown message (default: `'You do not have permission to manage this collection'`).
5. **`BookmarkConflictError.detail` + `CollectionConflictError.detail`** improvement: both were hardcoded `'Resource already exists'`; both now preserve thrown messages.
6. **`BookmarkValidationError.detail`** improvement: was hardcoded `'Invalid request data'`; now preserves thrown message.

Decisions:
1. **`BookmarkDomainError` stays abstract (no `code`, no mapping entry).** Same pattern as the prior 9 modules. Audit: `grep -rn 'new BookmarkDomainError' src/` returns no matches.
2. **`BookmarkDomainErrorDto` Swagger DTO deletion.** Third (and last until instance) module with such a DTO. The `bookmark-response-docs.dto.ts` filename was misleading because the file's original purpose was to host `Wrapped*Dto` success wrappers (since deleted in Phase 1). After Phase 1 it contained only `BookmarkDomainErrorDto`. Safe to delete the file outright. The 3 helper decorators in `BookmarkController` are simplified to `ProblemDetailDto` directly. No `getSchemaPath` import here (no `oneOf` schemas).
3. **Module-level filter provider removal.** Same as tournament, review — filter was registered as a module provider. `bookmark.module.ts` providers list is shortened; filter import is removed.
4. **No `bookmarkBadRequestResponse` helper exists.** Only class-validator 400s use raw `ApiBadRequestResponse` decorators directly (line 95 in the controller, etc.). The 7 helper decorators focus on 403/404/409 which are domain-error-driven; 400 is purely validation-driven.
5. **Logging parity.** Prior filter had no logging (like review). The global filter logs `event: 'http_client_error'` (warn) for 4xx and `event: 'http_server_error'` (error) for 5xx. Same severity buckets.
6. **`LEGACY_COMPAT` shim still deferred.** Same rationale as rev4.0 / rev4.1 / rev4.2 / rev4.3.

Verification: `tsc --noEmit` clean. ESLint clean on every touched file (3 prettier auto-fixes applied). Bookmark + common-errors unit tests pass (113 tests; +7 mapping + +47 bookmark spec). rfc7807 e2e suite passes (85 tests; +7 new bookmark cases). The 8 pre-existing tournament app-spec failures remain unchanged from `main`.

**v4.3 (2026-07-11).** Phase 2 partial — **review** shipped end-to-end. Fourth Phase-2 module migrated; 6 concrete exceptions → 4 status codes (400/403/404/409). Second module (after tournament) with an existing `*DomainErrorDto` Swagger DTO to delete. Concrete deliverables:
- `src/common/errors/problem-code-mapping.ts` extended with 6 REVIEW_* entries: 1 × 404 (`REVIEW_NOT_FOUND`), 1 × 403 (`REVIEW_FORBIDDEN`), 2 × 409 (`REVIEW_CONFLICT`, `REVIEW_ALREADY_REPORTED`), 2 × 400 (`REVIEW_VALIDATION`, `REVIEW_ATTEMPT_REQUIRED`).
- `src/common/errors/problem-code-mapping.spec.ts` extended with 6 new resolution assertions.
- `src/modules/review/domain/errors/review-domain.errors.ts` migrated. `ReviewDomainError` becomes an abstract namespace marker (no `code`). All 6 concrete classes preserve their existing default messages that reference the module's constants (`review.constants.ts`). Audit: `grep -rn 'new ReviewDomainError' src/` returns no matches.
- `src/modules/review/domain/errors/review-domain.errors.spec.ts` (new): 41 mapping-completeness tests iterating a 6-row table + 5 aggregate invariants. The total-count guard (`total === 6`) defends against accidental additions/removals.
- `src/modules/review/transport/filters/review-domain-exception.filter.ts` (deleted): the per-module `@Catch(ReviewDomainError)` filter is gone.
- `src/modules/review/dto/response/review-domain-error.dto.ts` (deleted): the per-module `*DomainErrorDto` Swagger DTO is gone.
- `src/modules/review/dto/response/index.ts` (updated): removed the `export * from './review-domain-error.dto'` re-export.
- `src/modules/review/review.module.ts`: removed the filter provider (`ReviewDomainExceptionFilter`) and its import. Same pattern as tournament — filter was module-level here.
- `src/modules/review/transport/controller/review.controller.ts`: removed `UseFilters` import, `ReviewDomainErrorDto` import, `ReviewDomainExceptionFilter` import, `@UseFilters(ReviewDomainExceptionFilter)` decoration. The 3 helper decorators (`reviewNotFoundResponse`, `reviewConflictResponse`, `reviewBadRequestResponse`) are simplified to emit `ProblemDetailDto` directly with `ErrorResponseExamples` wired in.
- `src/modules/review/transport/controller/quiz-review.controller.ts`: same edits plus the 4th helper (`reviewForbiddenResponse`) simplified. The `QuizAnalyticsError` comment (lines 128-129) was updated to reflect the new reality: every review error now flows through the global filter, so the envelope shape is uniform (no separate "domain-filtered errors below" path).
- `src/modules/review/transport/controller/user-review.controller.ts`: removed `UseFilters` import, `ReviewDomainExceptionFilter` import, `@UseFilters(ReviewDomainExceptionFilter)` decoration. **No DTO refs** in this file — its 401/500 errors come from `GlobalExceptionFilter` via `ApiAuth` / `ApiPublicErrors`. After Phase 2 the `@UseFilters` was redundant and removed.
- `src/modules/review/transport/controller/admin-review.controller.ts`: comment updated to remove the reference to the now-deleted per-module filter (the admin service still has no domain error classes).
- `test/rfc7807.e2e-spec.ts` extended with 6 review-coverage tests covering the full 4-status-code matrix.

Wire-shape changes (Phase 2 breaking):
1. **Envelope change.** Review error responses now follow the canonical RFC 7807 shape. The prior `{ statusCode, message, error }` envelope is fully gone.
2. **`ReviewNotFoundError.detail`** improvement: 3 throw sites pass `'Quiz not found'`; the prior filter rewrote them to `'Review not found'`. The global filter preserves the thrown message.
3. **`ReviewForbiddenError.detail`** improvement: was hardcoded `'You do not have permission to perform this action'`; now preserves thrown message (default: `'You do not have permission to manage this review'`, or distinct strings like `'You do not have permission to view analytics for this quiz'`).
4. **`ReviewConflictError.detail`** improvement: was hardcoded `'Resource already exists'`; now preserves thrown message (`'You have already reviewed this quiz'`).
5. **`ReviewAlreadyReportedError.detail`** improvement: was hardcoded `'You have already reported this review'`; now preserves thrown message (which happens to match the hardcoded default by default, but custom overrides now surface verbatim).
6. **`ReviewValidationError.detail` + `ReviewAttemptRequiredError.detail`** improvement: both were hardcoded `'Invalid request data'`; both now preserve thrown messages (`'You cannot vote on your own review'` and `'You must complete at least one attempt before reviewing this quiz'`).

Decisions:
1. **`ReviewDomainError` stays abstract (no `code`, no mapping entry).** Same pattern as the prior 8 modules. Audit: `grep -rn 'new ReviewDomainError' src/` returns no matches. No `*OperationFailedError` would be useful.
2. **`user-review.controller.ts` had no `ReviewDomainErrorDto` references** but still carried `@UseFilters(ReviewDomainExceptionFilter)`. The decoration is removed — redundant now that the global filter handles everything. This is the first Phase-2 module I've seen where a controller carries `@UseFilters` without using the corresponding `*DomainErrorDto`. Future audits should expect this pattern (a controller that uses the filter for type safety without swagger DTOs).
3. **`ReviewDomainErrorDto` Swagger DTO deletion.** Second (and last, until instance) module with such a DTO. The 7 helper decorators across `review.controller.ts` (3) and `quiz-review.controller.ts` (4) are all simplified to `ProblemDetailDto`. The `getSchemaPath` import is not used here (no `oneOf` schemas), unlike tournament.
4. **Module-level filter provider removal.** Same as tournament — filter was registered as a module provider. `review.module.ts` providers list is shortened; filter import is removed.
5. **Logging parity.** Prior filter had no logging (unlike tournament's). The global filter logs `event: 'http_client_error'` (warn) for 4xx and `event: 'http_server_error'` (error) for 5xx. Same severity buckets. Log query filters update.
6. **`LEGACY_COMPAT` shim still deferred.** Same rationale as rev4.0 / rev4.1 / rev4.2.

Verification: `tsc --noEmit` clean (after fixing the `dto/response/index.ts` re-export). ESLint clean on every touched file (4 prettier auto-fixes applied). Review + common-errors unit tests pass (100 tests; +6 mapping + +41 review spec). rfc7807 e2e suite passes (78 tests; +6 new review cases). The 8 pre-existing tournament app-spec failures remain unchanged from `main`.
