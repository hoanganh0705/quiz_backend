...
**Status (rev3.2):** **auth — SHIPPED**. The global-filter code resolution, the `ProblemCodeMapping`, and the auth module are all in place; the per-module `AuthDomainExceptionFilter` is deleted. Quiz, attempt, user remain.
