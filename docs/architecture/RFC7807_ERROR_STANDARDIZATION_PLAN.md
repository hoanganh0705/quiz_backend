...
**Status (rev3.5):** **auth, quiz, attempt, user — SHIPPED.** Auth shipped in rev3.2. Quiz shipped in rev3.3. Attempt shipped in rev3.4. User shipped in rev3.5: the global-filter code resolution now covers all four Phase-1 modules, the per-module `AuthDomainExceptionFilter`, `QuizDomainExceptionFilter`, `AttemptDomainExceptionFilter`, and `UserDomainExceptionFilter` are all deleted, and 15 AUTH_* + 13 QUIZ_* + 2 QUIZ_ANALYTICS_* + 11 ATTEMPT_* + 4 USER_* codes resolve through `ProblemCodeMapping`. Phase 1 complete.
