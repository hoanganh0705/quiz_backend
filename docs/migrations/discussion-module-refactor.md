...
All schema changes are shipped in a single hand-written migration file: `src/core/database/migrations/0020_discussion_to_comments.sql`. The migration:

1. Drops the legacy tables (`discussion_threads`, `discussion_thread_subscriptions`, `discussion_saved_threads`, the polymorphic `discussion_reports`, the polymorphic `discussion_votes`, and the legacy `discussion_comments` keyed by `thread_id`).
2. Drops the legacy enums (`discussion_vote_value`, `discussion_thread_status`, `discussion_report_status`).
3. Recreates `discussion_comments` as a flat, quiz-keyed table with a self-referential `parent_comment_id` (max two-level reply tree).
4. Creates `discussion_comment_votes` and `discussion_comment_reports`, each with its own enum.

No data archival: the legacy content was user-generated and low-density, and the rest of the system does not reference it. The `00XX` suffix corresponds to the `_journal.json` entry; the SQL file lives alongside the other migrations in `src/core/database/migrations/`. The accompanying meta snapshot (`0020_snapshot.json`) is registered in `meta/_journal.json`.

If a deployment needs to preserve the legacy content, snapshot the prior tables before applying the migration; the migration is not reversible.

The reconcile counter (`quizzes.discussion_comments_count`) is maintained by the runtime `CommentCounterReconcilerService` weekly cron, not by SQL — see `docs/modules/discussion.md`. The `0009_reconcile_discussion_counts.sql` file is kept as a no-op so the migration chain stays valid for databases provisioned before this refactor.
...
OpenAPI metadata (`@ApiTags`, `@ApiBearerAuth`, `@ApiOperation`, `@ApiResponse`, `@ApiProperty`) lives inline on the controllers and the request/response DTOs. There is no `transport/swagger/` subdirectory; the declarations sit next to the code they describe so the route → DTO → response cycle is read in one place. See §8.10 for the OpenAPI artefact contract.
...
- `COMMENT_DOMAIN_EVENT_BUS` and the `CommentDomainEventBus` implementation (for cross-module listener registration in Notification and Social).
- `CommentModeratorAuditService` (for tests and for any future cross-module caller that needs to record moderator actions without going through the application service).
- `QUIZ_EXISTENCE_PORT` and `USER_EXISTENCE_PORT` (re-exports so that adapter wiring stays visible without requiring consumers to import the adapter classes directly).

The repository port (`COMMENT_REPOSITORY_PORT`) is **not** exported. The post-refactor cross-module surface is the event bus, not the storage; the repository is consumed only by the domain service inside this module. The domain service is also not exported. Module-level state is not used.
...
The OpenAPI metadata is declared inline on the controllers and DTOs (no `transport/swagger/` subdirectory — see §8.1). Every endpoint declares:

- The OpenAPI tag (`@ApiTags('comments')`) and the success status with the correct response DTO schema.
...
`docs/generated/openapi.json` is regenerated by `pnpm generate:openapi` and committed alongside the change. There is no separate `discussion-openapi.spec.ts` unit-level test; the OpenAPI artefact is validated by the project's general `pnpm generate:openapi` pipeline and by the e2e tests that exercise the routes.
...
- **Tasks**:
  ...
  - `src/modules/discussion/transport/controller/comment.controller.spec.ts` (renamed from `discussion.controller.spec.ts`) — the controller tests are updated.
  ...
  - `src/modules/social/infrastructure/repositories/social.repository.spec.ts` — the social feed tests are updated.
  - `test/*.e2e-spec.ts` — the cross-module e2e tests are updated.
- **Out of scope**: a separate `comment-openapi.spec.ts` and `comment-timestamp.spec.ts` are not produced. The OpenAPI artefact is generated by `pnpm generate:openapi` and validated by the e2e tests; the timestamp projection is exercised by the `comment.repository.spec.ts` and the application-service spec. The wrapper DTO is covered by the standard `openapi-schemas.spec.ts` (no per-module entry needed).
