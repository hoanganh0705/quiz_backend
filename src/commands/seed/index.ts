...
import { runUsersSeed, runCategoriesSeed, runTagsSeed, runBadgesSeed } from './foundation';
...
import { runTournamentSeed, runInstanceSeed } from './scenarios';
...
const runFoundation = async (): Promise<void> => {
  ...
  for (const s of summaries) {
    console.log(
      `  ${s.domain}: inserted=${s.inserted}, updated=${s.updated}, skipped=${s.skipped}`,
    );
  }
};
