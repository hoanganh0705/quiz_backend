    ...
    reports: [
      {
        ...
        reason: 'spam',
        status: 'dismissed',
        reviewedByUsername: 'community_moderator',
      },
    ],
  },
];

export const runCommentSeed = async (): Promise<SeedSummary[]> => {
  ...
  await db.transaction(async (tx) => {
    ...
    for (const seed of COMMENT_SEEDS) {
      ...
      const [existing] = await tx
        .select({ commentId: commentRows.commentId })
        .from(commentRows)
        .where(eq(commentRows.commentId, seed.commentId))
        .limit(1);
      ...
      await tx.insert(commentRows).values({
      ...
      for (const vote of seed.votes) {
        ...
        const inserted = await tx
          ...
          .onConflictDoNothing()
          .returning({ voteId: commentVotes.voteId });
