...
CREATE TYPE "public"."discussion_comment_report_status" AS ENUM('open', 'reviewed', 'dismissed', 'actioned');--> statement-breakpoint
CREATE TYPE "public"."discussion_comment_vote_value" AS ENUM('upvote', 'downvote');--> statement-breakpoint
...
CREATE TYPE "public"."social_feed_activity_type" AS ENUM('badge_earned', 'badge_revoked', 'rank_milestone', 'peak_rank_achieved', 'tournament_joined', 'tournament_completed', 'tournament_won', 'comment_created', 'discussion_created', 'discussion_solved', 'quiz_completed', 'quiz_milestone', 'instance_created', 'instance_joined', 'instance_completed');--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE "discussion_comment_reports" (
	"report_id" uuid PRIMARY KEY DEFAULT uuidv7() NOT NULL,
	"reporter_id" uuid NOT NULL,
	"comment_id" uuid NOT NULL,
	"reason" text NOT NULL,
	"details" text,
	"status" "discussion_comment_report_status" DEFAULT 'open' NOT NULL,
	"reviewed_by_user_id" uuid,
	"reviewed_at" timestamp with time zone,
	"action_taken" boolean DEFAULT false NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "discussion_comment_reports_reason_nonblank" CHECK (length(btrim(reason)) > 0)
);
--> statement-breakpoint
CREATE TABLE "discussion_comment_votes" (
	...
	"user_id" uuid NOT NULL,
	"comment_id" uuid NOT NULL,
	"value" "discussion_comment_vote_value" NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
CREATE INDEX "idx_discussion_comment_reports_status_created" ON "discussion_comment_reports" USING btree ("status" enum_ops,"created_at" timestamptz_ops);--> statement-breakpoint
CREATE INDEX "idx_discussion_comment_reports_comment" ON "discussion_comment_reports" USING btree ("comment_id" uuid_ops);--> statement-breakpoint
CREATE UNIQUE INDEX "uq_discussion_comment_reports_reporter_comment" ON "discussion_comment_reports" USING btree ("reporter_id" uuid_ops,"comment_id" uuid_ops);--> statement-breakpoint
CREATE UNIQUE INDEX "uq_discussion_comment_votes_user_comment" ON "discussion_comment_votes" USING btree ("user_id" uuid_ops,"comment_id" uuid_ops);--> statement-breakpoint
CREATE INDEX "idx_discussion_comment_votes_comment" ON "discussion_comment_votes" USING btree ("comment_id" uuid_ops);--> statement-breakpoint
CREATE INDEX "idx_discussion_comments_quiz_created" ON "discussion_comments" USING btree ("quiz_id" uuid_ops,"created_at" timestamptz_ops) WHERE deleted_at IS NULL;--> statement-breakpoint
...
CREATE INDEX "idx_discussion_comments_quiz_parent_created" ON "discussion_comments" USING btree ("quiz_id" uuid_ops,"parent_comment_id" uuid_ops,"created_at" timestamptz_ops) WHERE deleted_at IS NULL;--> statement-breakpoint
