...
/**
 * Convert a raw `comments` row + pre-joined author into
...
function cursorDesc(cursor: CommentCursor | null | undefined) {
  ...
  return sql`(
    ${commentRows.createdAt} < ${cursor.createdAt}
    OR (
      ${commentRows.createdAt} = ${cursor.createdAt}
      AND ${commentRows.commentId} < ${cursor.id}
    )
  )`;
}
 ...
 * `idx_comments_quiz_parent_created` to satisfy the
...
export async function topLevelCommentScan(
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(commentRows.quizId, options.quizId),
        isNull(commentRows.parentCommentId),
...
export async function reportScan(
  ...
  if (options.cursor) {
    conditions.push(
      sql`(
        ${commentReports.createdAt} < ${options.cursor.createdAt}
        OR (
          ${commentReports.createdAt} = ${options.cursor.createdAt}
          AND ${commentReports.reportId} < ${options.cursor.id}
        )
      )`,
    );
  }
