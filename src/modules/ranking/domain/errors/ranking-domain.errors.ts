...
export class UserRankingNotFoundError extends RankingDomainError {
  constructor(userId: string) {
    super(
      `User ranking not found for user: ${userId}`,
...
export class RankCalculationError extends RankingDomainError {
  constructor(
    ...
    super(
      `Rank calculation failed for ${period}: ${reason}`,
      'RANK_CALCULATION_ERROR',
      { period, ...context },
    );
...
export class PeriodResetError extends RankingDomainError {
  constructor(
    ...
    super(
      `Period reset failed for ${period}: ${reason}`,
      'PERIOD_RESET_ERROR',
      { period, ...context },
    );
...
export class RankConsistencyError extends RankingDomainError {
  constructor(
    ...
    super(
      `Rank consistency issue (${issueType}): ${details}`,
      'RANK_CONSISTENCY_ERROR',
      { issueType, ...context },
    );
