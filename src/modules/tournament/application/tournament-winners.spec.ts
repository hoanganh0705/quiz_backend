...
describe('TournamentService getTournamentWinners', () => {
  ...
  it('passes custom limit to repository', async () => {
    ...
    expect(tournamentRepository.getWinners).toHaveBeenCalledWith({ tournamentId: 't-1', limit: 10 });
  });
  ...
  it('returns leaderboard for completed tournament', async () => {
    ...
    tournamentRepository.getTournamentById.mockResolvedValue({ tournamentId: 't-1', status: 'finished' });
    tournamentRepository.getWinners.mockResolvedValue([
      { rank: 1, userId: 'u-1', username: 'Anh', score: 980, avatarUrl: 'https://example.com/a.png' },
    ]);
