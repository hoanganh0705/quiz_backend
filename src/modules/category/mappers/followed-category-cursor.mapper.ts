...
interface FollowCursorPayload {
  followedAt: string;
  followId: string;
}

export class FollowedCategoryCursorMapper {
  private static readonly uuidPattern =
    /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

  static parse(cursor: string): FollowCursorPayload {
    const parsed = decodeBase64JsonCursor<FollowCursorPayload>(cursor);
  ...
  static serialize(payload: FollowCursorPayload): string {
