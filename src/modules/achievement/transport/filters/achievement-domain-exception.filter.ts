...
@Catch(AchievementDomainError)
export class AchievementDomainExceptionFilter implements ExceptionFilter {
  catch(exception: AchievementDomainError, host: ArgumentsHost): void {
  ...
  private mapToHttp(error: AchievementDomainError): { status: number; message: string } {
    if (error instanceof BadgeNotFoundError) {
      return { status: HttpStatus.NOT_FOUND, message: 'Badge not found' };
    }

    if (AchievementUserNotFoundError) {
