...
export type UnansweredDiscussionCursor = {
  createdAt: string;
  threadId: string;
};

export class UnansweredDiscussionCursorMapper {
  private static readonly uuidPattern =
    /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

  static parse(cursor: string): UnansweredDiscussionCursor {
    const parsed = decodeBase64JsonCursor<UnansweredDiscussionCursor>(cursor);
  ...
  static serialize(payload: UnansweredDiscussionCursor): string {
