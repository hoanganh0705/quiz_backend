...
export type QuizDiscussionCursor = {
  createdAt: string;
  threadId: string;
};

export class QuizDiscussionCursorMapper {
  private static readonly uuidPattern =
    /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

  static parse(cursor: string): QuizDiscussionCursor {
    const parsed = decodeBase64JsonCursor<QuizDiscussionCursor>(cursor);
  ...
  static serialize(payload: QuizDiscussionCursor): string {
